<template>
  ...
  <AtomsContainer>
    ...
    <router-link to="/" class="flex items-center space-x-3 rtl:space-x-reverse">
        <img src="@/assets/imgs/logo.webp" alt="5 FIVEX Store" class="mb-3" />
    </router-link>
    <div class="flex">
...
<MoleculesHeaderMenu v-if="menuShow" :menu="Menu" @closeMenu="openMenu()"  />
</template>
<script setup lang="ts">

import { service } from '~/services/api';
import { createMenuHierarchy } from '~/utils/menu';

const Menu = ref([]);
const data = await service('wc/v3/products/categories', { per_page: 100, hide_empty: true });
interface Menu {
  id: number;
  name: string;
  link: string;
...
Menu.value = createMenuHierarchy(data) as Menu[];
